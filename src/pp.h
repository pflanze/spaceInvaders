...
#define PP(p)									\
	pp_helper((p) ? &(p)->vtable : NULL, stderr, true)

#define PP_TO(p, out)							\
	pp_helper((p) ? &(p)->vtable : NULL, out, true)

// same as PP_TO but does not print a newline (thus usable as part of
// implementing a pp method):
#define PP_TO_(p, out)							\
	pp_helper((p) ? &(p)->vtable : NULL, out, false)


EXPORTED void pp_helper(const struct ObjectVTable *const*this,
						FILE *out,
						bool newline);
